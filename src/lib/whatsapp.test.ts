...
describe("buildWhatsAppUrl — wa variant", () => {
  ...
  it("includes phone and encoded text", () => {
    ...
    expect(url).toBe(
      "https://wa.me/15551234567?text=Hello%2C%20world!%0ANice%20to%20meet%20you.",
    );
  });
...
describe("buildWhatsAppUrl — share variant", () => {
  it("omits the phone entirely and keeps text", () => {
    expect(buildWhatsAppUrl({ variant: "share", text: "hi" })).toBe(
      "https://wa.me/?text=hi",
    );
  });
});
...
describe("buildWhatsAppUrl — shortCode variant", () => {
  it("routes to wa.me/message/<CODE>", () => {
    expect(
      buildWhatsAppUrl({ variant: "shortCode", shortCode: "abc-123" }),
    ).toBe("https://wa.me/message/ABC123");
  });
  it("returns base path when no code", () => {
    expect(buildWhatsAppUrl({ variant: "shortCode" })).toBe(
      "https://wa.me/message/",
    );
  });
});
